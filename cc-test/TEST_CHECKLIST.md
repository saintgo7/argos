...
**테스트 일자**: 2026-04-14  
**전체 결과**: 10/10 검증 완료 (8 PASS, 1 PASS with nuance, 1 IMPORTANT FINDING)
...
### 6. Skill: slash command vs Skill tool
`/skill-name` slash command로 실행 시 Skill tool이 발화되지 않음.
오직 `Skill` tool을 명시적으로 호출했을 때만 `tool_name: "Skill"` 이벤트 발생.
slash command 방식은 Argos에서 추적 불가.
