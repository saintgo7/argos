...
4. 기본값: https://www.argos-ai.xyz
```

기본 생성되는 config에는 `apiUrl` 필드가 포함되지 않는다 — 이 경우 기본값으로 자동 해석된다. `*argos-ai.xyz` 호스트를 가리키는 `apiUrl` 값은 읽는 시점에 무시되고 기본값으로 대체된다(stale 도메인 자동 마이그레이션).

셀프호스팅 팀은 `--api-url` 플래그로 초기화하면 `.argos/project.json`과 `~/.argos/config.json`에 해당 URL이 기록되며, 팀 전원이 자동으로 해당 인스턴스를 사용한다.
...
Base URL: `http://localhost:3001` (dev) / `https://www.argos-ai.xyz` (prod)  
