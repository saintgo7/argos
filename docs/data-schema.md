...
// ─── CliAuthSession ────────────────────────────────────────────────────────
// argos 실행 시 생성되는 단기 인증 세션 (10분 유효)
// CLI가 polling하여 완료 여부를 확인한다.

model CliAuthSession {
  id        String        @id @default(cuid())
  userId    String?
  status    CliAuthStatus @default(PENDING)
  token     String?       // 완료 시 발급된 JWT (CLI가 가져간 후 의미 없음)
  expiresAt DateTime
  createdAt DateTime      @default(now())
...
`events` 테이블이 주 볼륨. 1.8M 행/년은 PostgreSQL에서 인덱스 기반 쿼리로 충분히 처리 가능한 규모다. MVP에서 파티셔닝이나 아카이빙은 불필요.
