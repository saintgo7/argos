...
| 상태 | 동작 |
|---|---|
| 로그인 X + `project.json` X | GitHub 로그인 → org 생성 → 프로젝트 생성 → hook 주입 |
| 로그인 X + `project.json` O | GitHub 로그인 → org 자동 합류 |
...
#### 인증
- GitHub OAuth + JWT 기반 CLI 인증 (polling 방식).
- Bearer 토큰 검증 미들웨어.
...
#### CLI 인증 페이지
- `argos login` 실행 시 열리는 브라우저 페이지.
- GitHub OAuth 완료 후 "터미널로 돌아가세요" 메시지 표시.
...
### 배포 용이성
- 환경 변수(`DATABASE_URL`, `JWT_SECRET`, `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`)만 설정하면 어느 인프라에서도 동작해야 한다.
- `docker-compose.yml` 한 파일로 전체 스택을 로컬에서 실행 가능해야 한다.

### 보안
- JWT 토큰은 DB에서 revocation 관리.
- 사용자는 자신이 속한 org의 프로젝트 데이터만 접근 가능.
- GitHub OAuth 외 인증 방식은 MVP에서 지원하지 않는다.
