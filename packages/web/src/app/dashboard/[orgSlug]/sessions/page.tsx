...
function SessionsContent({
  ...
  const handleRowClick = (sessionId: string) => {
    router.push(`/dashboard/${orgSlug}/sessions/${sessionId}`)
  }
          ...
          >
            <thead className="bg-muted/40 border-b border-border text-xs text-muted-foreground">
              <tr>
                ...
                {showProjectColumn && (
                  <th className="text-left py-3 px-4 font-medium whitespace-nowrap">프로젝트</th>
                )}
...
export default function OrgSessionsPage() {
  ...
  return (
    <Suspense fallback={<Skeleton className="h-screen w-full" />}>
      <SessionsContent orgSlug={orgSlug} projectId={projectId} />
    </Suspense>
  )
}
