...
export default function OrgSessionDetailPage({
  ...
  const { orgSlug, sessionId } = use(params)
  const router = useRouter()

  const { data, isLoading, error, refetch } = useSessionDetail(orgSlug, sessionId)
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <Button variant="outline" onClick={() => router.push(`/dashboard/${orgSlug}/sessions`)}>
          ← Back to Sessions
        </Button>
